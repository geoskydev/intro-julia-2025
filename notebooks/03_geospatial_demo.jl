...
# ╔═╡ 88e6b32c-967d-11f0-0356-d59fa0340618
using Rasters, ArchGDAL, GeoDataFrames, Statistics, Plots

# ╔═╡ ee4c4748-38e9-45a0-96f1-ed4a333e1686
md"# Analisis Geoespacial con Julia
Julio Novoa, 2025

---
"

# ╔═╡ 00668d2c-a869-4e82-ba63-f214da23c858
begin
	# Leer raster desde URL
...
# ╔═╡ 75721168-785a-48aa-920e-4ccadbc9e19a
# Crear una vista de Ecuador
ecu = view(r, X(-81.0852191551 .. -75.1860925808), Y(-5.0186422241 .. 1.4629920115))

# ╔═╡ 9da67ed6-1e2c-485e-8d23-68f8d94c9c8b
# Reproyectar raster a EPSG:32717
ecu_utm = Rasters.resample(ecu, res=(1000,1000), method = :bilinear, crs=EPSG(32717))

# ╔═╡ e411f99a-4974-4621-adb1-2c7977cb1ae1
Plots.plot(
    ecu_utm,
    color=:thermal,
    title="Temperatura Promedio (1981-2010)",
    xlabel="UTM X (m)", ylabel="UTM Y (m)", colorbar_title="°C",
    aspect_ratio = :equal,
    size=(800,600)
)

# ╔═╡ dcbbcca2-bc0f-4960-b78d-80e4eccba3c9
begin
	# Importar cantones (poligonos) desde github
...
# ╔═╡ 202365bb-e709-4151-932b-cb28247c155d
begin
	# Estadísticas zonales: temperatura media anual por cantón
	temp_promedio = DataFrame(temp_promedio=zonal(mean, ecu_utm, of=cantones_poligonos; progress=false))
	insertcols!(cantones_poligonos, :temp_promedio => temp_promedio.temp_promedio, makeunique=true)
	first(cantones_poligonos, 5)
end

# ╔═╡ c7161e37-ccd1-4b07-9073-13d25aa1491c
cantones_poligonos |>
	df -> sort(df, :temp_promedio) |>
	df -> select(df, [:DPA_DESCAN, :DPA_DESPRO, :temp_promedio]) |>
	df -> first(df, 5)

# ╔═╡ eb2553e8-35a3-4a75-a994-25d60845be52
Plots.plot(
     ...
     linewidth = 0.2,
     linecolor = :black,
     title = "Temperatura Promedio (1981-2010), por canton",
     xlabel="UTM X (m)", ylabel="UTM Y (m)", colorbar_title = "°C",
     aspect_ratio = :equal,
     size=(800,600)
)
...
manifest_format = "2.0"
project_hash = "fd0971700f21de2f19d9c265130f8c1e235ba1dd"
...
[[deps.GeoDataFrames]]
deps = ["ArchGDAL", "DataAPI", "DataFrames", "Extents", "GeoFormatTypes", "GeoInterface", "GeometryOps", "Pkg", "Proj", "Reexport", "Tables"]
git-tree-sha1 = "9ece07dc4a5a7e58aa6381c2f5cf77f2389a9f9f"
...
git-tree-sha1 = "4f948c146b97c2dd62300bdf31a773c6301dc3c2"
...
[[deps.Rasters]]
deps = ["Adapt", "ColorTypes", "CommonDataModel", "ConstructionBase", "Dates", "DimensionalData", "DiskArrays", "Extents", "FillArrays", "Flatten", "GeoFormatTypes", "GeoInterface", "GeometryOpsCore", "LinearAlgebra", "Missings", "Mmap", "OffsetArrays", "ProgressMeter", "RecipesBase", "Reexport", "Setfield", "Statistics"]
git-tree-sha1 = "e741f9f9e51a7fd4527cf6df07251cc7a339ffc5"
